    ...
    <div class="col-md-10">
        ...
        <div class="table-search-v1 panel panel-grey">
            <div class="panel-heading">
                <h3 class="panel-title"><i class="fa fa-map-marker"></i> Location Update</h3>
            </div>
            <!-- Form -->
            {{ Form::open(array('action' => array('location.update', $data['location']->location_key), 'class' => 'sky-form', 'method' => 'PUT')) }}
                <fieldset>
                    <div class="row">
                        ...
                        <section class="col col-5">
                            <label class="input @if ($errors->has('location_name')) state-error @endif">
                                    ...
                                    <input type="text" name="location_name" value="{{ (Input::old('location_name')) ? Input::old('location_name') : $data['location']->location_name }}">
                    ...
                    <div class="row">
                        ...
                        <section class="col col-10">
                            <label class="textarea @if ($errors->has('location_address')) state-error @endif">
                                ...
                                <textarea rows="4" name="location_address">{{ (Input::old('location_address')) ? Input::old('location_address') : $data['location']->location_address }}</textarea>
                    ...
                    <div class="row">
                        ...
                        <section class="col col-10">
                            <label class="textarea @if ($errors->has('location_description')) state-error @endif">
                                <i class="icon-append fa fa-comment"></i>
                                <textarea rows="4" name="location_description">{{ (Input::old('location_description')) ? Input::old('location_description') : $data['location']->location_description }}</textarea>
                            ...
                            <label class="select @if ($errors->has('location_status')) state-error @endif">
                                <select name="location_status">
                                    @foreach ($data["status"] as $key => $value)
                                        ...
                                        @elseif (Input::old('location_status') == "" && $data['location']->location_status == $key)
                                            <option value="{{ $key }}" selected>{{ $value }}</option>
                                        @else
