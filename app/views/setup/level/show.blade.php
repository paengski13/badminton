    ...
    <div class="col-md-10">
        ...
        <div class="table-search-v1 panel panel-grey">
            <div class="panel-heading">
                <h3 class="panel-title"><i class="fa fa-level-up"></i> Level Update</h3>
            </div>
            <!-- Form -->
            {{ Form::open(array('action' => array('level.update', $data['level']->level_key), 'class' => 'sky-form', 'method' => 'PUT')) }}
                <fieldset>
                    <div class="row">
                        ...
                        <section class="col col-5">
                            <label class="input @if ($errors->has('level_name')) state-error @endif">
                                    ...
                                    <input type="text" name="level_name" value="{{ (Input::old('level_name')) ? Input::old('level_name') : $data['level']->level_name }}">
                    ...
                    <div class="row">
                        ...
                        <section class="col col-10">
                            <label class="textarea @if ($errors->has('level_description')) state-error @endif">
                                ...
                                <textarea rows="4" name="level_description">{{ (Input::old('level_description')) ? Input::old('level_description') : $data['level']->level_description }}</textarea>
                                {{ $errors->first('level_description', '<em for="level_name" class="invalid error-msg">:message</em>') }}
                            </label>
                        </section>
                    </div>
                </fieldset>
                    ...
                    <div class="row">
                        ...
                        <section class="col col-3">
                            <label class="input @if ($errors->has('level_sort')) state-error @endif">
                                    ...
                                    <input type="text" name="level_sort" value="{{ (Input::old('level_sort')) ? Input::old('level_sort') : $data['level']->level_sort }}">
                            ...
                            <label class="select @if ($errors->has('level_status')) state-error @endif">
                                <select name="level_status">
                                    @foreach ($data["status"] as $key => $value)
                                        ...
                                        @elseif (Input::old('level_status') == "" && $data['level']->level_status == $key)
                                            <option value="{{ $key }}" selected>{{ $value }}</option>
                                        @else
