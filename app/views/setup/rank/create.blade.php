    ...
    <div class="col-md-10">
        ...
        <div class="table-search-v1 panel panel-grey">
            <div class="panel-heading">
                <h3 class="panel-title"><i class="fa fa-graduation-cap"></i> Rank Add</h3>
            </div>
            <!-- Form -->
            {{ Form::open(array('action' => 'rank.store', 'class' => 'sky-form')) }}
                    ...
                    <div class="row">
                        ...
                        <section class="col col-5">
                            <label class="input @if ($errors->has('rank_name')) state-error @endif">
                                ...
                                <input type="text" name="rank_name" value="{{ Input::old('rank_name') }}">
                    ...
                    <div class="row">
                        ...
                        <section class="col col-10">
                            <label class="textarea @if ($errors->has('rank_description')) state-error @endif">
                                ...
                                <textarea rows="4" name="rank_description">{{ Input::old('rank_description') }}</textarea>
                                {{ $errors->first('rank_description', '<em for="rank_description" class="invalid error-msg">:message</em>') }}
                            </label>
                        </section>
                    </div>
                </fieldset>
                    ...
                    <div class="row">
                        ...
                        <section class="col col-2">
                            <label class="input @if ($errors->has('rank_sort')) state-error @endif">
                                ...
                                <input type="text" name="rank_sort" value="{{ (Input::old('rank_sort')) ? Input::old('rank_sort') : $data['sort_last_value'] }}">
                            ...
                            <label class="select @if ($errors->has('rank_status')) state-error @endif">
                                <select name="rank_status">
                                    @foreach ($data["status"] as $key => $value)
                                        ...
                                        @elseif (Input::old('rank_status') == "" && $key == 'A')
                                            <option value="{{ $key }}" selected>{{ $value }}</option>
                                        @else
