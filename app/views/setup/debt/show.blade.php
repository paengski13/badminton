    ...
    <div class="col-md-10">
        ...
        <div class="table-search-v1 panel panel-grey">
            <div class="panel-heading">
                <h3 class="panel-title"><i class="fa fa-money"></i> Debt Update</h3>
            </div>
            <!-- Form -->
            {{ Form::open(array('action' => array('debt.update', $data['debt']->debt_key), 'class' => 'sky-form', 'method' => 'PUT')) }}
                <fieldset>
                    <div class="row">
                        ...
                        <section class="col col-5">
                            <label class="input @if ($errors->has('debt_name')) state-error @endif">
                                    ...
                                    <input type="text" name="debt_name" value="{{ (Input::old('debt_name')) ? Input::old('debt_name') : $data['debt']->debt_name }}">
                    ...
                    <div class="row">
                        ...
                        <section class="col col-10">
                            <label class="textarea @if ($errors->has('debt_description')) state-error @endif">
                                ...
                                <textarea rows="4" name="debt_description">{{ (Input::old('debt_description')) ? Input::old('debt_description') : $data['debt']->debt_description }}</textarea>
                    ...
                    <div class="row">
                        ...
                        <section class="col col-3">
                            <label class="input @if ($errors->has('debt_sort')) state-error @endif">
                                    ...
                                    <input type="text" name="debt_sort" value="{{ (Input::old('debt_sort')) ? Input::old('debt_sort') : $data['debt']->debt_sort }}">
                            ...
                            <label class="select @if ($errors->has('debt_status')) state-error @endif">
                                <select name="debt_status">
                                    @foreach ($data["status"] as $key => $value)
                                        ...
                                        @elseif (Input::old('debt_status') == "" && $data['debt']->debt_status == $key)
                                            <option value="{{ $key }}" selected>{{ $value }}</option>
                                        @else
