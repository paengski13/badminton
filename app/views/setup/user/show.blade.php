...
@section("setup_content")
    <div class="col-md-10">
        ...
        <div class="table-search-v1 panel panel-grey">
            ...
            <!-- Form -->
            {{ Form::open(array('action' => array('user.update', $data['user']->user_key), 'class' => 'sky-form', 'id' => 'form', 'method' => 'PUT', 'files' => true)) }}
                <div class="tab-v2">
                    <ul class="nav nav-tabs">
                        <li class="active"><a href="#tab-profile" data-toggle="tab">Profile</a></li>
                        @if (Auth::user()->user_admin == 'Y')
                                            ...
                                            <select name="user_status">
                                                ...
                                                @foreach ($data["status"] as $key => $value)
                                                    @if (Input::old('user_status') == $key)
                                                        <option value="{{ $key }}" selected>{{ $value }}</option>
                                                    @elseif (Input::old('user_status') == "" && $key == $data['user']->user_status)
                                                        <option value="{{ $key }}" selected>{{ $value }}</option>
                                                    @else
                                                        <option value="{{ $key }}">{{ $value }}</option>
                                                    @endif
